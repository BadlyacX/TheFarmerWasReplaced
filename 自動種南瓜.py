...
def plant_crop_and_move(entities, ground_type, direction):
		plant_crop(entities, ground_type)
...
while True:
	if can_harvest() and num_items(Items.Hay) <= item_num_base:
		...
		move(East)
	elif can_harvest() and num_items(Items.Pumpkin) != item_num_base: # I write this deliberately
		for i in range(get_world_size()):
			plant_crop_and_move(Entities.Pumpkin, Grounds.Soil, North)
		move(East)
	else:
		plant_crop_and_move(Entities.Pumpkin, Grounds.Soil, North)
